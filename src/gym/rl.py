...
def _sample(obss, acts, qs, max_pts, is_reweight):
    ...
    if is_reweight:
        # According to p(s)
        idx = np.random.choice(obss.shape[0], size=min(max_pts, np.sum(ps > 0)), p=ps)
    else:
        # Uniformly (without replacement)
        idx = np.random.choice(obss.shape[0], size=min(max_pts, np.sum(ps > 0)), replace=False)    
...
def train_dagger(env, teacher, student, state_transformer, max_iters, n_batch_rollouts, max_samples, train_frac, is_reweight, n_test_rollouts):
    ...
    ob = np.array([obs for obs, _, _ in trace])
    teacher_qs=[]
    for i in range(ob.shape[0]):
        q = teacher.predict_q(ob[i])
        teacher_qs.append(q[0,0])
    qs.extend(np.array(teacher_qs))
    ...
    for i in range(max_iters):
        ...
        teacher_qs=[]
        for i in student_obss:
            q = teacher.predict_q(i)
            teacher_qs.append(q[0,0])
        ...
        #########
        #########
        student_obss = np.reshape(student_obss,(-1))
        #print(student_obss.shape)
        student_obss = np.reshape(student_obss,(-1,70))
        #print(student_obss)
        teacher_acts=[]
        for i in range(student_obss.shape[0]):
            act = teacher.predict(student_obss[i])
            #print(act)
            #print(act[0])
        ...
        #check teacher reward
        #teacher_trace = get_rollouts(env, teacher, False, n_batch_rollouts)
        #cur_rew = sum((rew for _, _, rew in teacher_trace)) / n_batch_rollouts
