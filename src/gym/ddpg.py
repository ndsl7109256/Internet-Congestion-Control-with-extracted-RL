...
class ActorNet(nn.Module):
    def __init__(self, state_dim=70, action_dim=1, hidden_dim=(400, 300)):
        super().__init__()
        h1, h2 = hidden_dim
...
class DDPG:
    def __init__(self, args):
        ...
        # action noise
        self._action_noise = GaussianNoise(dim=1)
        # memory
    ...
    def select_action(self, state, noise=True):
        ...
        with torch.no_grad():
            act = self._actor_net(to_state.float())
            if noise:
                act += torch.from_numpy(self._action_noise.sample()).view(1,-1).to(self.device)
    ...
    def _update_target_network(target_net, net, tau):
        ...
        for target, behavior in zip(target_net.parameters(), net.parameters()):
            tmp=target.data * (1-tau) + behavior.data * tau
...
def train(args, env, agent, writer):
    ...
    torch.save(arr,'ddpg_reward4000.pkt')
    env.close()
...
def main():
    ...
    parser.add_argument('-m', '--model', default='ddpg4000.pth')
    parser.add_argument('--logdir', default='log/ddpg')
    # train
    parser.add_argument('--warmup', default=2000000, type=int)
    ...
    parser.add_argument('--seed', default=20200420, type=int)
